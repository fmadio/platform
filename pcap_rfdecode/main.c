...
static void ProcessPacket(u8* Payload, u32 Length, u64 TS, u32 Flag)
{
	...
	// Check if the packet is a SDR packet
	bool PktIsSDR        = (Header->lock_status >> 6) & 0x1;

	// 100G packet contains 192 words (48xCGMII)
	// 10/25G packet contains 64 words (64xXGMII)
	int NumWords 		 = 32;

	if (g_Verbose) printf("%s cap%i SeqNo:%016llx CompressCnt:%8i CompressWord:%08x PktIsSDR:%d Timestamp:%s.%u Underflow:%4i Overflow:%4i FIFOError:%08x Gaps:%lli %s\n", 
										HeaderStr,
										Header->lane_no, 
										Header->seq_no, 
										Header->compress_cnt, 
	...
	if (PktIsSDR){
		...
		for (int w=0; w < NumWords; w++)
		{
			...
			if (s_IsPrintXGMII)
			{
				...
				printf("%s %3i : %02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x\n", 
							HeaderStr,
							w,
